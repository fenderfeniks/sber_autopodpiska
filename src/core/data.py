...
class UniversalDataLoader:
    ...
    def __init__(self, config: DictConfig, project_root, source_type: str = 'sql'):
        self.PROJECT_ROOT = project_root

        valid_sources = ["sql", "csv", "parquet"]
        if source_type not in valid_sources:
            raise ValueError(f"Неподдерживаемый source_type: '{source_type}'. Допустимые: {valid_sources}")
    ...
    def load_data(self, sql_file_name: str = "features/dirty_baseline_aggregation.sql") -> pd.DataFrame:
        """
        Универсальный загрузчик. Выбирает метод загрузки строго на основе self.source_type.
        """
        
        # ============================================================
        # ВЕТКА 1: РАБОТА С БАЗОЙ ДАННЫХ (SQL)
        # ============================================================
        if self.source_type == "sql":
            ...
            logger.info(f"[DB_MODE] Кэш не найден. Запуск агрегации в PostgreSQL (Версия фичей: {self.aggrigation_version})...")
            ...
            # Формируем динамическое SQL-условие для таргета
            ...
            # Подставляем параметры в шаблон SQL
            ...
            # Сэмплирование пользователей в СУБД для DEV-режима
        ...
        # ============================================================
        # ВЕТКА 2: РАБОТА С ЛОКАЛЬНЫМИ ФАЙЛАМИ (CSV / PARQUET)
        # ============================================================
        else:
            import duckdb
            # Определяем путь к файлу: либо передан напрямую через CLI, либо берем из дефолтных путей конфига
            ...
            logger.info(f"[FILE_MODE] Источник: {self.source_type.upper()}. Чтение файла: {file_path.name}")
            
            # --- Обработка CSV ---
            if file_path.suffix == '.csv':
                if self.sample_pct < 1.0:
                    logger.info(f"[DEV MODE] Быстрое сэмплирование CSV через DuckDB: {self.sample_pct * 100}%")
                    query = f"SELECT * FROM read_csv_auto('{file_path.as_posix()}') USING SAMPLE {self.sample_pct * 100} PERCENT (bernoulli)"
            ...
            # --- Обработка Parquet ---
                ...
                if self.sample_pct < 1.0:
                    ...
                    query = f"SELECT * FROM '{file_path.as_posix()}' USING SAMPLE {self.sample_pct * 100} PERCENT (bernoulli)"
                    df = duckdb.query(query).to_df()
                else:
                    df = pd.read_parquet(file_path)

        # ============================================================
        # ОБЩАЯ ПОСТ-ОБРАБОТКА: SMART ROW DROPPING
        # ============================================================
